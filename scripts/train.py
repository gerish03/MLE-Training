...
def training_model(processed_dataset_path, ml_model_path, log_level=logging.INFO, log_to_file=True):
    ...
    logger.info(f"ML model path: {ml_model_path}")

    X_train_path = os.path.join(processed_dataset_path, "X_train.csv")
    logger.info(f"Loading X_train data from: {X_train_path}")
    X_train = pd.read_csv(X_train_path)

    y_train_path = os.path.join(processed_dataset_path, "y_train.csv")
    logger.info(f"Loading y_train data from: {y_train_path}")
    ...
    logger.info("Training Randomized Search CV Random Forest model...")
    rand_tune_RF_model = rand_tune_random_forest(X_train, y_train)

    logger.info("Training Grid Search CV Random Forest model...")
    ...
    logger.info(sorted(zip(feature_importances, X_train.columns), reverse=True))
    ...
    dump(rand_tune_RF_model, os.path.join(ml_model_path, "rand_tune_RF_model.pkl"))
    dump(grid_tune_Tuned_RF_model, os.path.join(ml_model_path, "grid_tune_Tuned_RF_model.pkl"))
...
if __name__ == "__main__":
    ...
    training_model(args.processed_dataset_path, args.ml_model_path, log_level=log_level, log_to_file=log_to_file)
